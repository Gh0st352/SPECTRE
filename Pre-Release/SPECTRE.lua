...
SPECTRE._persistenceLocations = {
  ...
  _SavedState   = {folder = "SavedState", path = ""}
}
...
function SPECTRE:Persistence(MissionName)
  ...
  self._persistenceLocations._SavedState.path = MissionName .. "/".. self._persistenceLocations._SavedState.folder .. "/"
  ...
  if SPECTRE.IO.file_exists(self._masterProfileFile) then
    -- Load Stuff
    local _saveData = SPECTRE.IO.PersistenceFromFile(self._masterProfileFile)

    self._persistence = _saveData._persistence
    --self._loaded = _loaded,
    self._firstRun = _saveData._firstRun
    self._missionInProgress = _saveData._missionInProgress
    self._persistenceLocations = _saveData._persistenceLocations
    self.COUNTER = _saveData.COUNTER
    self.Coalitions = _saveData.Coalitions
    self.Countries = _saveData.Countries
    self.USERSTORAGE = _saveData.USERSTORAGE
    self.ZONEMGR = SPECTRE.UTILS.setTableValues(_saveData.ZONEMGR, self.ZONEMGR)
  ...
  SPECTRE.UTILS.debugInfo("  SavedState:     " .. self._persistenceLocations._SavedState.path)
  return self
end
...
SPECTRE.AI.restrictedPatrolTypes = {
  "Artillery",
  "Buildings",
  "Fortifications",
  --"AAA",
...
SPECTRE.AI._taskQueue = {}
SPECTRE.AI._taskSched = {}
SPECTRE.AI._taskSchedFlag = false
SPECTRE.AI._assigningTask = false
SPECTRE.AI.UpdateInterval = 10
SPECTRE.AI.UpdateIntervalNudge = 0.9
...
function SPECTRE.AI.posiFixSavedRoute(currentCoord, route)
  local posiFixedRoute = {}
  local numPoints = #route

  local patrolRoute = false
  local patrolRoute_nWaypointIndx = 0
  local patrolRoute_fromWaypointIndex = 0

  local lastPointParams = route[numPoints].task.params
  local lastPointTasks = lastPointParams.tasks
  local lastPointTasks_gotoIndex = 0

  if lastPointTasks then
    for _i,_v in ipairs(lastPointTasks) do
      if _v.id then
        if _v.id == "GoToWaypoint" then
          patrolRoute = true
          lastPointTasks_gotoIndex = _i
          patrolRoute_nWaypointIndx = _v.params.nWaypointIndx
          patrolRoute_fromWaypointIndex = _v.params.fromWaypointIndex
        end
      end
    end
  end

  local closestWPtoPosi = 0
  local closestWPtoPosiDistance = 99999999999

  for _i,_v in ipairs(route) do
    local tempDist = SPECTRE.POLY.distance({x = currentCoord.x, y = currentCoord.z}, {x = _v.x,y = _v.y})
    if tempDist < closestWPtoPosiDistance then
      closestWPtoPosi = _i
      closestWPtoPosiDistance = tempDist
    end
  end

  if not patrolRoute then

    if closestWPtoPosi > 1 then
      for _i = closestWPtoPosi, numPoints do
        posiFixedRoute[#posiFixedRoute + 1] = route[_i]
      end

      if #route[1].task.params.tasks > 0 then
        for _i, _v in ipairs(route[1].task.params.tasks) do
          posiFixedRoute[1].task.params.tasks[#posiFixedRoute[1].task.params.tasks+1] = _v
        end
      end
    end

  elseif patrolRoute then
    local posiTrackTemp = 0

    if closestWPtoPosi > 1 then

      for _i = closestWPtoPosi, numPoints do
        if _i == numPoints then posiTrackTemp = #posiFixedRoute + 1 end
        posiFixedRoute[#posiFixedRoute + 1] = route[_i]
      end
      for _i = 1, closestWPtoPosi - 1 do
        posiFixedRoute[#posiFixedRoute + 1] = route[_i]
      end
      ...
      posiFixedRoute[#posiFixedRoute].task.params.tasks = lastPointTasks
    end

  end


  if posiFixedRoute == {} then posiFixedRoute = route end
  return posiFixedRoute
end


SPECTRE.AI._templatePoint =  {
  ["speed_locked"] = true,
  ["type"] = "Turning Point",
  ["ETA"] = 0,
  ["ETA_locked"] = false,
...
SPECTRE.AI._templateTaskParams =  {
  ["action"] = {
    ["id"] = "Option",
    ["params"] = {
      ["value"] = 0,
...
SPECTRE.AI._templateTaskGoToWP = {
  ["enabled"] = true,
  ["auto"] = false,
  ["id"] = "GoToWaypoint",
  ["number"] = 1,
...
---.
function SPECTRE.AI:taskSchedInit()
  SPECTRE.UTILS.debugInfo("SPECTRE.AI:taskSchedInit | ---------------------")
  SPECTRE.AI._taskSchedFlag = true
  -- Initialize a scheduler to periodically check and update zones
  self._taskSched = SCHEDULER:New(self, function()
    local _counter = 0
    for i = #self._taskQueue, 1, -1 do
      local _task = self._taskQueue[i]
      if _task then
        if _task.Group:IsAlive() then
          _task.Group:Route(_task.TaskRoute)
        end
        table.remove(self._taskQueue, i)  -- Remove processed task
      end
      _counter = _counter + 1
      if _counter >= 3 then break end
    end
...
function SPECTRE.AI.taskPatrol(numWaypoints, speedMin, speedMax, patrolRadius, Group)
  local startPoint = Group:GetVec2()
  local unitName = Group:GetName()

  SPECTRE.UTILS.debugInfo("SPECTRE.AI.buildPatrol |  ------------------------------------")
  SPECTRE.UTILS.debugInfo("SPECTRE.AI.buildPatrol | GroupName          : ".. unitName)
  SPECTRE.UTILS.debugInfo("SPECTRE.AI.buildPatrol | numWaypoints          : ".. numWaypoints)
  SPECTRE.UTILS.debugInfo("SPECTRE.AI.buildPatrol | patrolRadius : ".. patrolRadius)
  SPECTRE.UTILS.debugInfo("SPECTRE.AI.buildPatrol | startPoint.x       : ".. startPoint.x .. " | startPoint.y : ".. startPoint.y )
  SPECTRE.UTILS.debugInfo("SPECTRE.AI.buildPatrol | speedMin: " .. speedMin)
  SPECTRE.UTILS.debugInfo("SPECTRE.AI.buildPatrol | speedMax  : " .. speedMax)


  local TaskRoute = {}
  local vec2_

  for _i = 1, numWaypoints, 1 do
    TaskRoute[_i] = SPECTRE.UTILS.deepcopy(SPECTRE.AI._templatePoint)

    if _i == 1 then
      vec2_ = startPoint
    else

      vec2_ = SPECTRE.WORLD.randPointInCircle(vec2_, patrolRadius)

      --      local tempZone = ZONE_RADIUS:New("_TempZonePatrol", startPoint, patrolRadius)
      --
      ...
      --      repeat
      --        flag_goodcoord = true
      --        vec2_ = tempZone:GetRandomVec2()
      --        if flag_goodcoord then flag_goodcoord = SPECTRE.SPAWNER:vec2AtNoGoSurface(vec2_) end
      --        if glassBreak >= opLimit then
      --          SPECTRE.UTILS.debugInfo("SPECTRE.AI.buildPatrol | --- GLASSBREAK --")
      --          flag_goodcoord = true
      --        end
    ...
    TaskRoute[_i].speed = math.random(speedMin, speedMax)
    TaskRoute[_i].x = vec2_.x
    TaskRoute[_i].y = vec2_.y

    SPECTRE.UTILS.debugInfo("SPECTRE.AI.buildPatrol | TaskRoute " .. _i .. " : Speed " .. TaskRoute[_i].speed)
    SPECTRE.UTILS.debugInfo("SPECTRE.AI.buildPatrol | TaskRoute " .. _i .. " Point.x       : ".. TaskRoute[_i].x .. " | Point.y : ".. TaskRoute[_i].y )

    if _i == numWaypoints then
      TaskRoute[_i].task.params.tasks[#TaskRoute[_i].task.params.tasks+1] = SPECTRE.UTILS.deepcopy(SPECTRE.AI._templateTaskGoToWP)
      TaskRoute[_i].task.params.tasks[#TaskRoute[_i].task.params.tasks].params.fromWaypointIndex = numWaypoints-1
      TaskRoute[_i].task.params.tasks[#TaskRoute[_i].task.params.tasks].params.nWaypointIndx = 1
    end

  end

  --SPECTRE.IO.PersistenceToFile("SPECTRE_TEST/" .. "taskRoute.lua", TaskRoute, true)

  table.insert(SPECTRE.AI._taskQueue, {Group = Group, TaskRoute = TaskRoute})
  --Group:Route(TaskRoute)

  SPECTRE.MISSION.dB.GroupTaskings[unitName] = TaskRoute

  return TaskRoute
end

function SPECTRE.AI.taskPatrol_Route(numWaypoints, speedMin, speedMax, patrolRadius, Group)
  local startPoint = Group:GetVec2()
  local unitName = Group:GetName()
...
function SPECTRE.AI.taskPatrol_Groups(numWaypoints_min, numWaypoints_max, speed_min, speed_max, patrolRadius_min, patrolRadius_max, patrolChance, Groups, delay)
  local delay = delay or 0
  if SPECTRE.DebugEnabled == 1 then
    SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | ---------------------------------")
    SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | numWaypoints_min : " .. numWaypoints_min)
    SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | numWaypoints_max : " .. numWaypoints_max)
    SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | speed_min        : " .. speed_min)
    SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | speed_max        : " .. speed_max)
    SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | patrolRadius_min : " .. patrolRadius_min)
    SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | patrolRadius_max : " .. patrolRadius_max)
    SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | patrolChance     : " .. patrolChance)
    SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | delay            : " .. delay)
    SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | #Groups          : " .. #Groups)
    SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | ---------------------------------")
  end
  ...
  if #Groups > 0 then
    ...
    local _Timer = TIMER:New(function()
      for i, group_ in ipairs(Groups) do
        local _groupName = group_:GetName()
        if SPECTRE.DebugEnabled == 1 then
          SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | Group Index      : " .. i)
          SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | Group Name       : " .. _groupName)
          SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | Group Data       : " , group_)
        end
        -- if group_.attributes.TaskRoute ~= nil and group_.attributes.DynamicRoute ~= nil then
        local Types = SPECTRE.UTILS.GetGroupAttributes(_groupName)
        SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | Group Types      : " , Types)
        if not SPECTRE.UTILS.isTableValueInTable(SPECTRE.AI.restrictedPatrolTypes, Types) then
          local addRoute = SPECTRE.UTILS.rollChance_TrueFalse(patrolChance)
          SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | Group Route?     : " .. tostring(addRoute))
          if addRoute then
            if SPECTRE.DebugEnabled == 1 then
              NumPatrols = NumPatrols + 1
            end
            SPECTRE.AI.taskPatrol(math.random(numWaypoints_min, numWaypoints_max), speed_min, speed_max, math.random(patrolRadius_min, patrolRadius_max), group_)
          end
          --    end
        end
      end
      if SPECTRE.DebugEnabled == 1 then
        SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | ---------------------------------")
        SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | totalGroups          : " .. tostring(totalGroups))
        SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | NumPatrols           : " .. tostring(NumPatrols))
        SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | Num No Patrols       : " .. tostring(totalGroups-NumPatrols))
        SPECTRE.UTILS.debugInfo("SPECTRE.AI.taskPatrol_Groups | % Patrols            : " .. tostring((NumPatrols/totalGroups)*100) .. "%")
      end
    end, numWaypoints_min, numWaypoints_max, speed_min, speed_max, patrolRadius_min, patrolRadius_max, patrolChance, Groups)
    _Timer:Start(delay)
...
--- **BUBBLE**
--
-- Bubble Spawner and Object Management for SPECTRE.
--
-- ===
--
-- @module BUBBLE
...
-- @field #MISSION
SPECTRE.MISSION = {}
SPECTRE.MISSION.SaveUnits_ = false
SPECTRE.MISSION.RestartTimer_ = {}
SPECTRE.MISSION.RestartTimer_.Main = {}
SPECTRE.MISSION.RestartTimer_.Warnings = {}
SPECTRE.MISSION.RestartTimer_.WarningPercents = {1,5,10,25}
SPECTRE.MISSION.dB = {}
SPECTRE.MISSION.dB.GroupTaskings = {}


--timeToRestart = min
function SPECTRE.MISSION.RestartTimer(timeToRestart_Seconds, restartUserFlag)


  for _,_percent in pairs(SPECTRE.MISSION.RestartTimer_.WarningPercents) do
    local time = timeToRestart_Seconds * (_percent/100)
    local time_seconds = time
    local timeOutText = time
    local unitTypeTime = "seconds"
    ...
    if time >=60 and time < 3600 then
      unitTypeTime = "minutes"
      timeOutText = time / 60
    end

    SPECTRE.MISSION.RestartTimer_.Warnings[#SPECTRE.MISSION.RestartTimer_.Warnings +1] = TIMER:New(function()
      trigger.action.outText("Server Restarts in " .. timeOutText .. " " .. unitTypeTime .. ".", 30)
    end):Start(timeToRestart_Seconds - time_seconds)
  end


  if timeToRestart_Seconds > 60 then
    SPECTRE.MISSION.RestartTimer_.Warnings[#SPECTRE.MISSION.RestartTimer_.Warnings +1] = TIMER:New(function()
      trigger.action.outText("Server Restarts in 1 minute!", 30)
      trigger.action.outText("You do NOT need to disconnect.", 30)
...
--        __group:Destroy(true)
    ...
    end):Start(timeToRestart_Seconds - 60)
  end

  SPECTRE.MISSION.RestartTimer_ = TIMER:New(function()
      ...
      --CONCEPTUAL
      --      -- Iterate through all units in all groups and reset their tasks
      --for _, coalition in pairs({coalition.side.RED, coalition.side.BLUE, coalition.side.NEUTRAL}) do
      --    local groups = coalition.getGroups(coalition)
      ...
      trigger.action.outText("Server is restarting now!!", 600)
      trigger.action.setUserFlag(restartUserFlag, 1)
      --SPECTRE.MISSION.RestartTimer_:Stop()
      return
...
SPECTRE.SPAWNER.Patrols = false
SPECTRE.SPAWNER.PatrolsConfig = {}
SPECTRE.SPAWNER.PatrolsConfig.Chance = 0
SPECTRE.SPAWNER.PatrolsConfig.numWaypoints = 0
SPECTRE.SPAWNER.PatrolsConfig.speedMin = 0
SPECTRE.SPAWNER.PatrolsConfig.speedMax = 0
SPECTRE.SPAWNER.PatrolsConfig.patrolRadius = 0
  ...
  for _groupSettingIndex, setting in ipairs(_zoneObject.GroupSettings) do
    ...
    for _groupIndex = 1, setting.numGroups do
      ...
      for _Unit = 1, setting.size + _zoneObject.SpawnAmountSettings.numExtraUnits do --+ self.Config.numExtraUnits
        ...
        if self.Patrols == true then
          _spawnunit:OnSpawnGroup(function(_spawnunit)
            local addRoute = SPECTRE.UTILS.rollChance_TrueFalse(self.PatrolsConfig.Chance)
            if addRoute then
              SPECTRE.AI.taskPatrol_Route(self.PatrolsConfig.numWaypoints, self.PatrolsConfig.speedMin, self.PatrolsConfig.speedMax, self.PatrolsConfig.patrolRadius, _spawnunit)
            end
          end)
        end
...
function SPECTRE.SPAWNER.SpawnSavedGroups(_Groups)
  for _index, _group in ipairs(_Groups) do
    local _attributes = _group.attributes
    local _PrimeUnit = _group.Units[1]

    if _attributes.TemplateDonor then
      local _TemplateName = _attributes.TemplateDonor
      local _SpawnName = SPECTRE.UTILS.removeGroupNumberIdentifier(_group.Name)
      local _SpawnHeading = _PrimeUnit.Heading
      local _SpawnCoord = COORDINATE:New(_PrimeUnit.Coordinate.x,_PrimeUnit.Coordinate.y,_PrimeUnit.Coordinate.z)
      local _SpawnCountry = _attributes.Country
      local _SpawnCoalition = _attributes.Coalition
      local _SavedRoute
      if not _attributes.DynamicRoute then
        _SavedRoute = _attributes.TaskRoute
      else
        _SavedRoute = _attributes.DynamicRoute
      end
      local _PosiFixedRoute = SPECTRE.AI.posiFixSavedRoute(_SpawnCoord, _SavedRoute)

      SPECTRE.UTILS.debugInfo("SPECTRE.SPAWNER.SpawnSavedGroups | GROUP ------------------------------------")
      SPECTRE.UTILS.debugInfo("SPECTRE.SPAWNER.SpawnSavedGroups | Name          : ".. _SpawnName)
      SPECTRE.UTILS.debugInfo("SPECTRE.SPAWNER.SpawnSavedGroups | _TemplateName : ".. _TemplateName)
      SPECTRE.UTILS.debugInfo("SPECTRE.SPAWNER.SpawnSavedGroups | Coord.x       : ".. _SpawnCoord.x .. " | Coord.y : ".. _SpawnCoord.y .. " | Coord.z : ".. _SpawnCoord.z)
      SPECTRE.UTILS.debugInfo("SPECTRE.SPAWNER.SpawnSavedGroups | spawnCoalition: " .. tostring(_SpawnCoalition))
      SPECTRE.UTILS.debugInfo("SPECTRE.SPAWNER.SpawnSavedGroups | spawnCountry  : " .. tostring(_SpawnCountry))


      local _spawnunit = SPAWN:NewWithAlias(_TemplateName, _SpawnName)
        :InitCoalition(_SpawnCoalition)
        :InitCountry(_SpawnCountry)
        :InitHeading(_SpawnHeading)
        :InitPositionCoordinate(_SpawnCoord)
        :OnSpawnGroup(function(_spawnunit)
          _spawnunit:Route(_PosiFixedRoute)
        end)
        :Spawn()
    end
  end
...
-- @usage local modifiedString = SPECTRE.UTILS.removeGroupNumberIdentifier("SPEC_Ground Units_467#001") -- Returns "SPEC_Ground Units_467".
function SPECTRE.UTILS.removeGroupNumberIdentifier(str)
  -- Pattern matches a '#' followed by one or more digits at the end of the string
  local pattern = "#%d+$"
  -- Replace the matched pattern with an empty string to remove it
  local modifiedStr = string.gsub(str, pattern, "")
  return modifiedStr
end
...
---.
function SPECTRE.WORLD.getGroupsForCoalition(coalitionSide, groupType)
...
function SPECTRE.WORLD.getGroupsForCoalition2(coalitionSide, groupType)
  ...
  for _,_group in ipairs(_Groups) do
    ...
    workingGroup.Name = Group.getName(_group)

    local MOOSEGROUP = GROUP:FindByName(workingGroup.Name)
...
---.
function SPECTRE.WORLD.getGroundGroupsInZone(ZoneName, coalition)
  local Groups_ = {}
  local Zone_ = ZONE:FindByName(ZoneName)
  local ZoneSet = SET_GROUP:New()
    :FilterZones({Zone_})
...
---.
function SPECTRE.WORLD.randPointInCircle(center, radius)
  local vec2_ = {}
  local tempZone = ZONE_RADIUS:New("_TempZonePatrol", center, radius)
