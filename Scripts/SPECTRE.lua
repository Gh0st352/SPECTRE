...
function SPECTRE.DynamicSpawner:DetermineCoordinates()
  ...
  self:Set_Vec2_Types()
  local T3=os.time()
  BASE:E(string.format("Dynamic - FindObjects Time: %d",T1-T0))
  BASE:E(string.format("Dynamic - Set_Vec2_GroupCenters Time: %d",T2-T1))
  BASE:E(string.format("Dynamic - Set_Vec2_Types Time: %d",T3-T2))
  BASE:E(string.format("Dynamic - DetermineCoordinates Time: %d",T3-T0))
  return self
end
    ...
    for _j = 1, #self.Zones.Sub[_i].GroupSettings, 1 do
      for _k = 1, self.Zones.Sub[_i].GroupSettings[_j].NumberGroups, 1 do
        ...
        while flag_goodcoord == 0 do
          ...
          for _ke,_v in pairs(self.Zones.Sub[_i].ObjectCoords) do
            ...
            if _ke == "groupcenters" then
              for _ii = 1, #self.Zones.Sub[_i].ObjectCoords[_ke] do
                counter_operation = SPECTRE.DynamicSpawner.CO_Yield(counter_operation, self.Config.operationLimit)
                local checkCoord = self.Zones.Sub[_i].ObjectCoords[_ke][_ii]
                if DEBUG == 1 then
                  BASE:E("DEBUG - Set_Vec2_GroupCenters - self.Zones.Sub[_i].ObjectCoords")
                  BASE:E(self.Zones.Sub[_i].ObjectCoords)
                  BASE:E("DEBUG - Set_Vec2_GroupCenters - _k")
                  BASE:E(_k)
                  BASE:E("DEBUG - Set_Vec2_GroupCenters - _ke")
                  BASE:E(_ke)
  ...
  for _j = 1, #self.Zones.Main.GroupSettings, 1 do
    for _k = 1, self.Zones.Main.GroupSettings[_j].NumberGroups, 1 do
      ...
      while flag_goodcoord == 0 do
        ...
        --check if coord too close to restricted
        for _ke,_v in pairs(self.Zones.Main.ObjectCoords) do
          --set distance restriction
          local distance
          if _ke == "units" then
          ...
          if _ke == "groupcenters" then
            for _ii = 1, #self.Zones.Main.ObjectCoords[_ke] do
              counter_operation = SPECTRE.DynamicSpawner.CO_Yield(counter_operation, self.Config.operationLimit)
              local checkCoord = self.Zones.Main.ObjectCoords[_ke][_ii]
              if DEBUG == 1 then
      ...
      for _k = 1, self.Zones.Sub[_i].GroupSettings[_j].NumberGroups, 1 do
        ...
        for _typeNum = 1, self.Zones.Sub[_i].GroupSettings[_j].GroupSize, 1 do
          ...
          while flag_goodcoord == 0 do
            ...
            --check if coord too close to restricted
            for _ke,_v in pairs(self.Zones.Sub[_i].BuiltSpawner[_j][_k].ObjectCoords) do
              --set distance restriction
              local distance
              if _ke == "units" then
                distance = self.Zones.Sub[_i].GroupSettings[_j].minSeperation
              ...
              for _ii = 1, #self.Zones.Sub[_i].BuiltSpawner[_j][_k].ObjectCoords[_ke] do
                counter_operation = SPECTRE.DynamicSpawner.CO_Yield(counter_operation, self.Config.operationLimit)
                local checkCoord = self.Zones.Sub[_i].BuiltSpawner[_j][_k].ObjectCoords[_ke][_ii]--self.Zones.Sub[_i].ObjectCoords[_ke][_ii]
                if DEBUG == 1 then
                  BASE:E("DEBUG - Set_Vec2_Types - self.Zones.Sub[_i].BuiltSpawner[_j][_k].ObjectCoords")
                  BASE:E(self.Zones.Sub[_i].BuiltSpawner[_j][_k].ObjectCoords)
                  BASE:E("DEBUG - Set_Vec2_Types (Sub) - _k")
                  BASE:E(_k)
                  BASE:E("DEBUG - Set_Vec2_Types (Sub) - _ke")
                  BASE:E(_ke)
                  BASE:E("DEBUG - Set_Vec2_Types (Sub) - _i")
                  BASE:E(_i)
                  BASE:E("DEBUG - Set_Vec2_Types (Sub) - _ii")
                  BASE:E(_ii)
                  BASE:E("DEBUG - Set_Vec2_Types (Sub) - checkCoord")
                  BASE:E(checkCoord)
                  BASE:E("DEBUG - Set_Vec2_Types (Sub) - possibleVec2")
                  BASE:E(possibleVec2)
                  BASE:E("DEBUG - Set_Vec2_Types (Sub) - distance")
                  BASE:E(distance)
                  BASE:E("DEBUG - Set_Vec2_Types (Sub) - f_distance")
                  BASE:E(SPECTRE.f_distance(checkCoord,possibleVec2))
                end
                if SPECTRE.f_distance(checkCoord,possibleVec2) < distance then --and SPECTRE.f_distance(checkCoord,possibleVec2) < distancemax then
                  flag_goodcoord = 0
                  break
                end
...
function SPECTRE.DynamicSpawner:Spawn()
  local DEBUG = 1
  local counter_operation = 0

  if DEBUG == 1 then
    BASE:E("DEBUG - Spawn - ENTER")
  end

  for _i = 1, #self.Zones.Sub, 1 do
    self.Zones.Sub[_i].TypeCounter = 1
    for _j = 1, #self.Zones.Sub[_i].GroupSettings, 1 do
      for _k = 1, self.Zones.Sub[_i].GroupSettings[_j].NumberGroups, 1 do
        ...
        for _m = 1, self.Zones.Sub[_i].GroupSettings[_j].GroupSize, 1 do
          ...
          local _spawnunit = SPAWN:NewWithAlias(template_, name_ .. "_" .. self.Zones.Sub[_i].TypeCounter)
            :InitHeading(0,364)
            :SpawnFromVec2(vec2_)
          self.Zones.Sub[_i].BuiltSpawner[_j][_k].ActivatedUnits[_m] = _spawnunit
          self.Zones.Sub[_i].TypeCounter = self.Zones.Sub[_i].TypeCounter + 1
        end
      end
    end
  end

  self.Zones.Main.TypeCounter = 1
  for _j = 1, #self.Zones.Main.GroupSettings, 1 do
    for _k = 1, self.Zones.Main.GroupSettings[_j].NumberGroups, 1 do
      ...
      for _m = 1, self.Zones.Main.GroupSettings[_j].GroupSize, 1 do
        ...
        local _spawnunit = SPAWN:NewWithAlias(template_, name_ .. "_" .. self.Zones.Main.TypeCounter)
          :InitHeading(0,364)
          :SpawnFromVec2(vec2_)
        self.Zones.Main.BuiltSpawner[_j][_k].ActivatedUnits[_m] = _spawnunit
        self.Zones.Main.TypeCounter = self.Zones.Main.TypeCounter + 1
      end
    end
  end
...
function SPECTRE.DynamicSpawner:DEBUG_PRINT_self()
  local DEBUG = 1
  if DEBUG == 1 then
    BASE:E("DEBUG - DEBUG_PRINT_self - ")
    for _k,_v in pairs(self) do
      BASE:E("DEBUG - DEBUG_PRINT_self - pairs(self)")
      BASE:E("key")
      BASE:E(_k)
      if (type(_v) == "table") then
        BASE:E("value")
        BASE:E(self[_k])
        for _k2,_v2 in pairs(self[_k]) do
          BASE:E("DEBUG - DEBUG_PRINT_self - pairs(self[_k])")
          BASE:E("key2")
          BASE:E(_k2)
          if (type(_v2) == "table") then
            BASE:E("value2")
            BASE:E(self[_k][_k2])
            for _k3,_v3 in pairs(self[_k][_k2]) do
              BASE:E("DEBUG - DEBUG_PRINT_self - pairs(self[_k][_k2])")
              BASE:E("key3")
              BASE:E(_k3)
              if (type(_v3) == "table") then
                BASE:E("value3")
                BASE:E(self[_k][_k2][_k3])
              else
                BASE:E("value3")
                BASE:E(_v3)
              end
