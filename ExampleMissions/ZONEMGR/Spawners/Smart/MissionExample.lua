...
---CONTINUED FROM `ZONEMGR BasicSpawner`
...
-- spawn initial units for SMART example
zoneManager:spawnAirbasesInZone("Zone_DELTA",SPECTRE.Coalitions.Blue,SPECTRE.Countries.Blue,template_BLUEspawner)

--- The SMART spawner allows the mission to spawn with a bias towards a certain coalition, allowing intelligent spawning.


---  Smart Spawn Timer for example.
--  Not needed if units already exist in a zone.
--  This is to allow our airbase spawn above time to finish spawning.
--
--  If you "Smart spawn" with a bias towards a coalition that does not exist in the zone, odds are unexpected behavior will happen.
--
local _SpawnTimer1 = TIMER:New(function()
  -- Here, we are spawning our template, and telling the mission to spawn in "Zone Delta" with a bias for placing units near existing Blue units.
  zoneManager:spawnFillInZoneSmart("Zone_DELTA",SPECTRE.Coalitions.Red,SPECTRE.Countries.Red,nil,SPECTRE.Coalitions.Blue,template_REDspawner)
end)
_SpawnTimer1:Start(10)

local _SpawnTimer2 = TIMER:New(function()
  -- Here, we are spawning our template, and telling the mission to spawn in "Zone Delta" with a bias for placing units near existing Red units.
  zoneManager:spawnFillInZoneSmart("Zone_DELTA",SPECTRE.Coalitions.Blue,SPECTRE.Countries.Blue,nil,SPECTRE.Coalitions.Red,template_BLUEspawner)
end)
