...
---CONTINUED FROM `IADS BasicSetup`
...
SPECTRE.ZONEMGR:enablePersistance()               -- Enables the ZONEMGR to save data, preventing the need to perform the same operations on subsequent mission starts.
  :setUpdateInterval(20, 0.15)                   -- Sets how often the ZONEMGR will analyze the managed zones and update itself.

zoneManager = SPECTRE.ZONEMGR:New():Setup(zoneNames):Init()

redIADS = SPECTRE.IADS:New(1,zoneNames):createSkynet("REDIADS"):DebugOn():setSkynetUpdateInterval(5):setUpdateInterval(25, 0.25):Init()
blueIADS = SPECTRE.IADS:New(2,zoneNames):createSkynet("BLUEIADS"):DebugOn():setSkynetUpdateInterval(5):setUpdateInterval(25, 0.25):Init()
